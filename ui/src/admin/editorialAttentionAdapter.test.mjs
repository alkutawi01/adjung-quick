...
console.log('\nEDITORIAL ATTENTION — V1 evaluation tests\n');
...

{
  const items = evaluateEditorialAttention({
    classifications: [{ story_id: 'low', classification_confidence: 0.49 }],
  }, now);
  assert('confidence below the existing 0.5 threshold creates action_required',
    items.length === 1 && items[0].type === 'low_confidence' && items[0].category === 'action_required');
}
{
  const items = evaluateEditorialAttention({
    classifications: [{ story_id: 'at-threshold', classification_confidence: 0.5 }],
  }, now);
